...
Do not invent a missing base or head. If there are multiple repositories, ambiguous names, ambiguous remotes, or an unresolved ref, show the candidates and ask the user to choose. Quick mode uses the current local branch's `HEAD`, does not require that branch to be pushed, and rejects detached `HEAD`.
...
Add `--remote <name>` when the repository has multiple remotes, `--include-working-tree` to review saved local changes, and `--no-fetch` only when the local remote-tracking ref is intentionally authoritative. When including working-tree files, pass a fresh `--context-dir` outside the repository when the caller controls the temporary path; otherwise the helper creates a durable external context bundle automatically and returns its path. Remove that bundle after the review report is complete. The helper uses `git ls-remote --symref <remote> HEAD` to find the remote default branch. `--base <remote>/<branch>` is an explicit quick-mode override and cannot be combined with `--remote`.

The helper discovers `.git` directories and worktree `.git` files, resolves workspace folders, fetches only the remote branch associated with the selected base, and then freezes the inputs to commit SHAs. It never pulls, checks out, resets, merges, or changes the user's index. A fetch failure is fatal; never silently review stale remote refs. Pure commit IDs and local branches without an upstream are not fetched. If a branch exists on several remotes, use an explicit remote ref such as `origin/release` or ask the user to choose.
